      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          {features.map((feature) => (
            <div
              key={feature.title}
              className="bg-white dark:bg-card border border-[#1B9AAA]/10 rounded-3xl p-6 hover:shadow-[0_6px_24px_rgba(27,154,170,0.10)] transition-shadow flex flex-col gap-4"
            >
              <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${feature.bg}`}>
                <feature.icon className={`h-6 w-6 ${feature.color}`} />
              </div>
              <div className="flex flex-col gap-2">
                <h3 className="text-xl font-semibold text-[#4A4A4A] dark:text-foreground">{feature.title}</h3>
              ...
              <a
                href={feature.link}
                className="text-sm font-medium text-[#1B9AAA] hover:underline mt-auto"
              >
                더 보기 →
              </a>
            </div>
